...
//#include "sdcard.h"
//RGB16位(565)颜色
uint16_t palettes[65] = {29614, 8401, 21, 16403, -30706, -22526, -24576, 30784, 16736, 544, 640, 482, 6635, 0, 32, 32,
...
byte flag_merge() {
    return (flag_n << 7) | (flag_v << 6) | 0x20 | (flag_b << 4) | (flag_d << 3) | (flag_i << 2) | (flag_z << 1) |
           flag_c;
}
    ...
    for (int line = lineStartIndex * 120; line < (lineStartIndex + 1) * 120; line++) {
        ...
        for (int i = 0; i < 32; i++) {
            ...
            for (int j = 0; j < 8; j++) {
                ...
                index = index - lineStartIndex * 256 * 120;
                if(index<0){
                    index = 0;
                }
...
void renderSprite(int sl) {
    ...
    if(get_bit(p_reg_2002,6)) return;
    ...
    for (int i = 0; i < 256; i += 4) {
        ...
        if (sl >= y && sl < y + spriteHeight) {
            ...
            byte colorData = p_read(spritePatternAddr + 8);
            //命中非透明背景 sprite0 hit
            if (i==0&&(spritePatternData&colorData)) {
...
void NES_Start() {
    ...
    LCD_SetRegion(35, 0, 256, LCD_HEIGHT);
    int i=0;
    while (1) {
        for (int l = 0; l < 240; l++) {
            if (showBg() || showSpr()) {
        ...
        if ((showBg() || showSpr())&&i==1) {
            LCD_CS_CLR
            LCD_DC_SET
            for (int i = 0; i < 2; ++i) {
                render_name_table(i);
                for (int j = 0; j < 30720; ++j) {
                    LCD_WR_POINT(render[j]);
                }
            }
            LCD_CS_SET
