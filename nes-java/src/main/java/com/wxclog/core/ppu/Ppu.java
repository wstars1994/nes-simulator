...
public class Ppu {
     ...
     * 开始绘制扫描线
     */
    public void preRender(int scanLineIndex, int[] renderBuff) {
        //渲染背景
        if(DataBus.showBg()) {
        ...
        //渲染精灵
        if(DataBus.showSpr()) {
            this.renderSprite(scanLineIndex,renderBuff);
        }
    }

    public void renderNameTable(int line,int[] render) {
        short p_vram_addr = DataBus.p_vram_addr;
        byte fine_y = (byte) ((p_vram_addr >> 12) & 7);
        byte fine_x = DataBus.p_scroll_x;
        short patternStartAddr = (short) ((DataBus.p_2000[4] == 0 ? 0x0000 : 0x1000)+fine_y);
        ...
        for (int i=0;i<32;i++) {
            ...
            //图案表颜色数据
            byte patternColorData = ppuMemory.read(patternAddress + 8);
            //取颜色低两位
    ...
    public void renderSprite(int line,int[] render) {
        byte bgColorIndex = ppuMemory.read(0x3F00);
